...
static int choosePixelFormat(_GLFWwindow* window, const _GLFWfbconfig* desired)
{
    ...
    {
        const int n = i + 1;
        _GLFWfbconfig* up_ = usableConfigs + usableCount;
        ...
        if (_glfw.wgl.ARB_pixel_format)
        {
            ...
            up_->redBits = getPixelFormatAttrib(window, n, WGL_RED_BITS_ARB);
            up_->greenBits = getPixelFormatAttrib(window, n, WGL_GREEN_BITS_ARB);
            up_->blueBits = getPixelFormatAttrib(window, n, WGL_BLUE_BITS_ARB);
            up_->alphaBits = getPixelFormatAttrib(window, n, WGL_ALPHA_BITS_ARB);

            up_->depthBits = getPixelFormatAttrib(window, n, WGL_DEPTH_BITS_ARB);
            up_->stencilBits = getPixelFormatAttrib(window, n, WGL_STENCIL_BITS_ARB);

            up_->accumRedBits = getPixelFormatAttrib(window, n, WGL_ACCUM_RED_BITS_ARB);
            up_->accumGreenBits = getPixelFormatAttrib(window, n, WGL_ACCUM_GREEN_BITS_ARB);
            up_->accumBlueBits = getPixelFormatAttrib(window, n, WGL_ACCUM_BLUE_BITS_ARB);
            up_->accumAlphaBits = getPixelFormatAttrib(window, n, WGL_ACCUM_ALPHA_BITS_ARB);

            up_->auxBuffers = getPixelFormatAttrib(window, n, WGL_AUX_BUFFERS_ARB);

            if (getPixelFormatAttrib(window, n, WGL_STEREO_ARB))
                up_->stereo = GLFW_TRUE;
            if (getPixelFormatAttrib(window, n, WGL_DOUBLE_BUFFER_ARB))
                up_->doublebuffer = GLFW_TRUE;

            if (_glfw.wgl.ARB_multisample)
                up_->samples = getPixelFormatAttrib(window, n, WGL_SAMPLES_ARB);
        ...
        else
        {
            ...
            up_->redBits = pfd.cRedBits;
            up_->greenBits = pfd.cGreenBits;
            up_->blueBits = pfd.cBlueBits;
            up_->alphaBits = pfd.cAlphaBits;

            up_->depthBits = pfd.cDepthBits;
            up_->stencilBits = pfd.cStencilBits;

            up_->accumRedBits = pfd.cAccumRedBits;
            up_->accumGreenBits = pfd.cAccumGreenBits;
            up_->accumBlueBits = pfd.cAccumBlueBits;
            up_->accumAlphaBits = pfd.cAccumAlphaBits;

            up_->auxBuffers = pfd.cAuxBuffers;
