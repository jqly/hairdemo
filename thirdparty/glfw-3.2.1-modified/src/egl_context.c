                                ...
                                EGLConfig* result)
{
    ...
    for (i = 0;  i < nativeCount;  i++)
    {
        ...
        up_->redBits = getEGLConfigAttrib(n, EGL_RED_SIZE);
        up_->greenBits = getEGLConfigAttrib(n, EGL_GREEN_SIZE);
        up_->blueBits = getEGLConfigAttrib(n, EGL_BLUE_SIZE);

        up_->alphaBits = getEGLConfigAttrib(n, EGL_ALPHA_SIZE);
        up_->depthBits = getEGLConfigAttrib(n, EGL_DEPTH_SIZE);
        up_->stencilBits = getEGLConfigAttrib(n, EGL_STENCIL_SIZE);

        up_->samples = getEGLConfigAttrib(n, EGL_SAMPLES);
        up_->doublebuffer = GLFW_TRUE;

        up_->handle = (uintptr_t) n;
        usableCount++;
    }
