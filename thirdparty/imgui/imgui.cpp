...
void ImGui::ColorConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b)
{
    ...
    int   i = (int)h;
    float forward_ = h - (float)i;
    float p = v * (1.0f - s);
    float q = v * (1.0f - s * forward_);
    float t = v * (1.0f - s * (1.0f - forward_));
...
void* ImFileLoadToMemory(const char* filename, const char* file_open_mode, size_t* out_file_size, int padding_bytes)
{
    ...
    FILE* forward_;
    if ((forward_ = ImFileOpen(filename, file_open_mode)) == NULL)
        return NULL;

    long file_size_signed;
    if (fseek(forward_, 0, SEEK_END) || (file_size_signed = ftell(forward_)) == -1 || fseek(forward_, 0, SEEK_SET))
    {
        fclose(forward_);
        return NULL;
    }
    ...
    if (fread(file_data, 1, file_size, forward_) != file_size)
    {
        fclose(forward_);
...
bool ImGuiTextFilter::PassFilter(const char* text, const char* text_end) const
{
    ...
    for (int i = 0; i != Filters.Size; i++)
    {
        const TextRange& forward_ = Filters[i];
        if (forward_.empty())
            continue;
        if (forward_.front() == '-')
        {
            // Subtract
            if (ImStristr(text, text_end, forward_.begin()+1, forward_.end()) != NULL)
                return false;
        }
        else
        {
            // Grep
            if (ImStristr(text, text_end, forward_.begin(), forward_.end()) != NULL)
                return true;
        }
    }
...
void ImGui::SaveIniSettingsToDisk(const char* ini_filename)
{
    ...
    FILE* forward_ = ImFileOpen(ini_filename, "wt");
    if (!forward_)
        return;
    fwrite(ini_data, sizeof(char), ini_data_size, forward_);
    fclose(forward_);
}
...
static inline float SliderBehaviorCalcRatioFromValue(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float power, float linear_zero_pos)
{
    ...
    if (is_power)
    {
        if (v_clamped < 0.0f)
        {
            const float forward_ = 1.0f - (float)((v_clamped - v_min) / (ImMin((TYPE)0, v_max) - v_min));
            return (1.0f - ImPow(forward_, 1.0f/power)) * linear_zero_pos;
        }
        else
        {
            const float forward_ = (float)((v_clamped - ImMax((TYPE)0, v_min)) / (v_max - ImMax((TYPE)0, v_min)));
            return linear_zero_pos + ImPow(forward_, 1.0f/power) * (1.0f - linear_zero_pos);
        }
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    float forward_[4] = { col[0], col[1], col[2], alpha ? col[3] : 1.0f };
    if (flags & ImGuiColorEditFlags_HSV)
        ColorConvertRGBtoHSV(forward_[0], forward_[1], forward_[2], forward_[0], forward_[1], forward_[2]);
    int i[4] = { IM_F32_TO_INT8_UNBOUND(forward_[0]), IM_F32_TO_INT8_UNBOUND(forward_[1]), IM_F32_TO_INT8_UNBOUND(forward_[2]), IM_F32_TO_INT8_UNBOUND(forward_[3]) };
    ...
    if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            if (flags & ImGuiColorEditFlags_Float)
                value_changed = value_changed_as_float = value_changed | DragFloat(ids[n], &forward_[n], 1.0f/255.0f, 0.0f, hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][n]);
            else
    ...
    if (picker_active_window == NULL)
    {
        if (!value_changed_as_float)
            for (int n = 0; n < 4; n++)
                forward_[n] = i[n] / 255.0f;
        if (flags & ImGuiColorEditFlags_HSV)
            ColorConvertHSVtoRGB(forward_[0], forward_[1], forward_[2], forward_[0], forward_[1], forward_[2]);
        if (value_changed)
        {
            col[0] = forward_[0];
            col[1] = forward_[1];
